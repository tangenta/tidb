...
func (db *DB) ExecDDL(ctx context.Context, ddlJob *model.Job) error {
	...
	case model.ActionCreateTable:
		...
		err = db.se.CreateTable(ctx, ddlJob.GetSchemaName(), infoCloned)
		if err != nil {
...
func (db *DB) ensurePlacementPolicy(ctx context.Context, policyName ast.CIStr, policies *sync.Map) error {
	if policies == nil {
		return nil
	}
