...
func (m *mockManager) IsOwner() bool {
	...
	return util.MockGlobalStateEntry.Type(m.key).IsOwner(m.id)
}

func (m *mockManager) toBeOwner() {
	ok := util.MockGlobalStateEntry.Type(m.key).SetOwner(m.id)
	if ok {
...
func (m *mockManager) retireOwner() {
	util.MockGlobalStateEntry.Type(m.key).UnsetOwner(m.id)
}
		...
		for {
			...
			default:
				...
				logutil.BgLogger().Debug("[ddl] owner manager tick", zap.String("ID", m.id),
					zap.String("ownerKey", m.key), zap.String("currentOwner", util.MockGlobalStateEntry.Type(m.key).GetOwner()))
			}
		}
	}()
	return nil
