...
# get tidb code with barnch name from gitea.
function get_tidb_code(){
    ...
    if [[ -n $test_pr ]]; then
        ...
        cd ..
	    cd $tidb_ee_test_dir
	    git fetch git@git.pingcap.net:pingkai/tidb-ee-test.git pull/$test_pr/head:pr_$test_pr
	    git checkout pr_$test_pr
	    git pull --rebase git@git.pingcap.net:pingkai/tidb-ee-test.git $tidb_ee_test_branch
   fi
