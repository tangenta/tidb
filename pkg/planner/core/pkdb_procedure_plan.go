...
func (b *PlanBuilder) buildCreateProcedure(ctx context.Context, node *ast.CreateProcedureInfo) (base.Plan, error) {
	...
	if procedurceSchema == "" {
		...
		node.ProcedureName.Schema = ast.NewCIStr(b.ctx.GetSessionVars().CurrentDB)
	}
	if procedurceSchema == "" {
...
func (b *PlanBuilder) buildDropProcedure(ctx context.Context, node *ast.DropProcedureStmt) (base.Plan, error) {
	...
	if procedurceSchema == "" {
		procedurceSchema = b.ctx.GetSessionVars().CurrentDB
		node.Name.Schema = ast.NewCIStr(b.ctx.GetSessionVars().CurrentDB)
	}
	if procedurceSchema == "" {
		return nil, plannererrors.ErrNoDB
	}
...
func (b *PlanBuilder) savePlanToCache(routineName ast.CIStr, plan *ProcedurePlan, lastChange types.Time) (*RoutineCacahe, error) {
...
func (b *PlanBuilder) getplanCache(ctx context.Context, routineName ast.CIStr, lastChange types.Time) (*ProcedurePlan, *RoutineCacahe, error) {
	key := b.ctx.GetSessionVars().CurrentDB + "." + routineName.O
