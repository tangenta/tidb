...
func (b *PlanBuilder) ensureRoutineSchema(name *ast.TableName) (string, error) {
	...
	if routineSchema == "" {
		...
		name.Schema = ast.NewCIStr(routineSchema)
	}
	if routineSchema == "" {
...
func (b *PlanBuilder) preprocessDropProcedure(node *ast.DropProcedureStmt) error {
	...
	if node.IsFunction {
		...
		if routineSchema == "" {
			...
			if currentDB != "" && b.is != nil {
				// If a stored function exists in current DB, treat it as stored routine; otherwise fall back to UDF.
				if _, ok := b.is.RoutineByName(ast.NewCIStr(currentDB), node.Name.Name, node.Type()); ok {
...
func routineCacheKey(schema, name ast.CIStr) string {
	return schema.L + "." + name.L
}

func (b *PlanBuilder) savePlanToCache(routineSchema, routineName ast.CIStr, plan *ProcedurePlan, lastChange types.Time) (*RoutineCacahe, error) {
...
func (b *PlanBuilder) getplanCache(ctx context.Context, routineSchema, routineName ast.CIStr, lastChange types.Time) (*ProcedurePlan, *RoutineCacahe, error) {
	key := routineCacheKey(routineSchema, routineName)
...
func (b *PlanBuilder) makePlanForCallProcedure(ctx context.Context, procedureInfo *ProcedurebodyInfo, sqlModeSave string, node *ast.CallStmt) (*ProcedurePlan, error) {
	...
	err = b.ctx.GetSessionVars().SetSystemVar(vardef.SQLModeVar, procedureInfo.SQLMode)
	defer func() {
		restoreErr := b.ctx.GetSessionVars().SetSystemVar(vardef.SQLModeVar, sqlModeSave)
		if restoreErr != nil {
...
func (b *PlanBuilder) tryBuildProcedureInstant(ctx context.Context, node ast.Node) (base.Plan, error) {
	...
	collate, ok := b.ctx.GetSessionVars().GetSystemVar(vardef.CollationConnection)
	if !ok {
...
func (b *PlanBuilder) buildCallProcedure(ctx context.Context, node *ast.CallStmt, skipPlanCache bool) (outplan base.Plan, err error) {
	...
	if procedurceSchema == "" {
		...
		node.Procedure.Schema = ast.NewCIStr(b.ctx.GetSessionVars().CurrentDB)
	}
	if procedurceSchema == "" {
	...
	sqlModeSave, ok := b.ctx.GetSessionVars().GetSystemVar(vardef.SQLModeVar)
	if !ok {
		return nil, errors.Errorf("can not find %s", vardef.SQLModeVar)
	}
...
func (b *PlanBuilder) fetchProcdureInfo(name, db, tp string) (*ProcedurebodyInfo, error) {
	if b.is != nil {
		if routine, ok := b.is.RoutineByName(ast.NewCIStr(db), ast.NewCIStr(name), tp); ok {
			procedureBodyInfo := &ProcedurebodyInfo{
				Name:                routine.Name.O,
				Procedurebody:       " CREATE " + tp + " " + routine.Name.O + "(" + routine.ParameterStr + ") \n" + routine.DefinitionUTF8,
				SQLMode:             routine.SQLMode,
				CharacterSetClient:  routine.CharacterSetClient,
				CollationConnection: routine.CollationConnection,
				ShemaCollation:      routine.SchemaCollation,
				SecurityType:        routine.SecurityType,
				lastChangeTime:      routine.LastAltered,
			}
