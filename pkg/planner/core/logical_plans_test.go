...
func TestVisitInfo(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if tt.isPointGetPlan {
			...
			require.Nil(t, TryFastPlan(s.GetCtx(), nodeW))
		}
...
func TestColumnPrivilegeVisitInfo(t *testing.T) {
	...
	for _, tt := range tests {
		...
		nodeW := resolve.NewNodeW(stmt)
		require.NoError(t, Preprocess(context.Background(), s.GetSCtx(), nodeW, WithPreprocessorReturn(&PreprocessorReturn{InfoSchema: s.GetIS()})))
		sctx := coretestsdk.MockContext()
		builder, _ := NewPlanBuilder().Init(sctx, s.GetIS(), hint.NewQBHintHandler(nil))
		domain.GetDomain(sctx).MockInfoCacheAndLoadInfoSchema(s.GetIS())
		builder.ctx.GetSessionVars().SetHashJoinConcurrency(1)
