...
func (j waitRegionsInit) wait(
	...
	sourcePDCli, err := pd.NewClientWithContext(runCtx, "log-repl", sourcePDAddrs, pd.SecurityOption{})
	if err != nil {
		return err
	}
...
func (w waitDataSync) wait(runCtx context.Context, sourcePDAddrs []string) error {
	...
	sourcePDCli, err := pd.NewClientWithContext(ctx, "log-repl", sourcePDAddrs, pd.SecurityOption{})
	if err != nil {
		return err
	}
	defer sourcePDCli.Close()
...
func pkdbBatchScanAllRegions(ctx context.Context, pdCli pd.Client) ([]*router.Region, error) {
	...
	for {
		batch, err := pdCli.BatchScanRegions(
			ctx,
			[]router.KeyRange{{StartKey: nextKey}},
...
type regionScanner interface {
	BatchScanRegions(ctx context.Context, ranges []router.KeyRange, limit int, opts ...opt.GetRegionOption) ([]*router.Region, error)
}
...
func checkPartRegionConsistency(startKey []byte, regions []*router.Region) error {
...
func (l loadRaftIndex) checkAndWait(ctx context.Context, newJobBytes []byte, lastFinishedVersion uint64, _ kv.Storage, _ *clientv3.Client) (version uint64, err error) {
	...
	primaryPDCli, err := pd.NewClientWithContext(ctx, "log-repl", pdAddrs, pd.SecurityOption{})
	if err != nil {
		return 0, err
	}
...
func getStandbyInitPercentage(regions []*router.Region, states []*logreplicationpb.LogReplicationState) float64 {
	done := 0
