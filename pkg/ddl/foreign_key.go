...
func checkTableForeignKeysValid(sctx sessionctx.Context, is infoschema.InfoSchema, schema ast.CIStr, tbInfo *model.TableInfo) error {
...
func checkTableForeignKeyValid(is infoschema.InfoSchema, schema ast.CIStr, tbInfo *model.TableInfo, fk *model.FKInfo, fkCheck bool) error {
...
func checkModifyColumnWithForeignKeyConstraint(is infoschema.InfoSchema, dbName ast.CIStr, tbInfo *model.TableInfo, originalCol, newCol *model.ColumnInfo) error {
...
func checkTableHasForeignKeyReferred(is infoschemactx.MetaOnlyInfoSchema, schema, tbl ast.CIStr, ignoreTables []ast.Ident, fkCheck bool) *model.ReferredFKInfo {
	if !fkCheck {
		return nil
	}
...
func checkTruncateTableHasForeignKeyReferredInOwner(infoCache *infoschema.InfoCache, job *model.Job, tblInfo *model.TableInfo, fkCheck bool) error {
	referredFK, err := checkTableHasForeignKeyReferredInOwner(infoCache, job.GetSchemaName(), job.GetTableName(), []ast.Ident{{Name: tblInfo.Name, Schema: ast.NewCIStr(job.SchemaName)}}, fkCheck)
	if err != nil {
		return err
	}
	if referredFK != nil {
...
func checkTableHasForeignKeyReferredInOwner(infoCache *infoschema.InfoCache, schema, tbl ast.CIStr, ignoreTables []ast.Ident, fkCheck bool) (_ *model.ReferredFKInfo, _ error) {
	if !vardef.EnableForeignKey.Load() {
		return nil, nil
	}
...
func checkIndexNeededInForeignKey(is infoschema.InfoSchema, dbName ast.CIStr, tbInfo *model.TableInfo, idxInfo *model.IndexInfo) error {
	referredFKs := is.GetTableReferredForeignKeys(dbName, tbInfo.Name)
...
func checkIndexNeededInForeignKeyInOwner(infoCache *infoschema.InfoCache, job *model.Job, dbName ast.CIStr, tbInfo *model.TableInfo, idxInfo *model.IndexInfo) error {
	if !vardef.EnableForeignKey.Load() {
		return nil
	}
	is := infoCache.GetLatest()
...
func checkDropColumnWithForeignKeyConstraint(is infoschema.InfoSchema, dbName ast.CIStr, tbInfo *model.TableInfo, colName string) error {
	for _, fkInfo := range tbInfo.ForeignKeys {
...
func checkAddForeignKeyValid(is infoschema.InfoSchema, schema ast.CIStr, tbInfo *model.TableInfo, fk *model.FKInfo, fkCheck bool) error {
	if !vardef.EnableForeignKey.Load() {
		return nil
	}
	err := checkTableForeignKeyValid(is, schema, tbInfo, fk, fkCheck)
...
func checkAddForeignKeyValidInOwner(infoCache *infoschema.InfoCache, schema ast.CIStr, tbInfo *model.TableInfo, fk *model.FKInfo, fkCheck bool) error {
