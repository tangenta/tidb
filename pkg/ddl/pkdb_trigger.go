...
func (e *executor) DropTrigger(ctx sessionctx.Context, stmt *ast.DropTriggerStmt) error {
	...
	var schemaName, triggerName ast.CIStr
...
func buildTriggerInfo(ctx sessionctx.Context, stmt *ast.CreateTriggerStmt) (*model.TriggerInfo, error) {
	...
	chs, ok := sessCtx.GetSystemVar(vardef.CharacterSetClient)
	if !ok {
		return nil, errors.New("unknown system var " + vardef.CharacterSetClient)
	}
	collConn, ok := sessCtx.GetSystemVar(vardef.CollationConnection)
	if !ok {
		return nil, errors.New("unknown system var " + vardef.CollationConnection)
	}
	dbColl, ok := sessCtx.GetSystemVar(vardef.CollationDatabase)
	if !ok {
		return nil, errors.New("unknown system var " + vardef.CollationDatabase)
	}
...
func checkDatabaseTriggerConflict(ctx context.Context, is infoschema.InfoSchema, schemaName ast.CIStr, triggerName ast.CIStr) error {
...
func onCreateTrigger(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	schema := ast.NewCIStr(job.SchemaName)
...
func findTriggerByName(tblInfo *model.TableInfo, triggerName ast.CIStr) *model.TriggerInfo {
	for _, trigger := range tblInfo.Triggers {
		if trigger.Name.L == triggerName.L {
			return trigger
		}
...
func removeTriggerByName(tblInfo *model.TableInfo, triggerName ast.CIStr) {
