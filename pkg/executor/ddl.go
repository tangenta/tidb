...
func (e *DDLExec) executeDropDatabase(s *ast.DropDatabaseStmt) error {
	...
	if currentDB.L == "" {
		currentDB = ast.NewCIStr(sessionVars.CurrentDB)
	}
	if err == nil && model.NameEqual(currentDB, dbName) {
		sessionVars.CurrentDB = ""
		sessionVars.CurrentDBCI = ast.CIStr{}
...
func (e *DDLExec) getRecoverTableByTableName(tableName *ast.TableName) (*model.Job, *model.TableInfo, error) {
	...
	if schemaName == "" {
		...
		if currentDB.L == "" {
			currentDB = ast.NewCIStr(e.Ctx().GetSessionVars().CurrentDB)
		}
...
func (e *DDLExec) dropProcedure(schemaName ast.CIStr) error {
