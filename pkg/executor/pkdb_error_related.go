...
func (e *SignalExec) Next(ctx context.Context, _ *chunk.Chunk) error {
	...
	for _, signalCon := range e.SignalCons {
		...
		col := &model.ColumnInfo{Name: ast.NewCIStr(name), FieldType: *types.NewFieldType(mysql.TypeVarString)}
...
func (e *GetDiagnosticsExec) Next(ctx context.Context, req *chunk.Chunk) error {
	...
	if e.Con != nil {
		...
		col := &model.ColumnInfo{Name: ast.NewCIStr(""), FieldType: *types.NewFieldType(mysql.TypeLong)}
