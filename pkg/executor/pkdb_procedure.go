...
func (e *ProcedureExec) triggerCallProcedure(ctx context.Context) (err error) {
	...
	mockCallStmt := &ast.CallStmt{
		Procedure: &ast.FuncCallExpr{
			Schema: ast.NewCIStr(e.Ctx().GetSessionVars().CurrentDB),
			FnName: ast.NewCIStr(""),
			Args:   []ast.ExprNode{},
		},
	}
...
func (e *ShowExec) fetchShowCreateProcdure(ctx context.Context, tp ast.ShowStmtType) error {
	if e.Procedure.Schema.O == "" {
		e.Procedure.Schema = ast.NewCIStr(e.Ctx().GetSessionVars().CurrentDB)
	}
...
func (b *executorBuilder) buildTriggerProcedure(v *plannercore.TriggerProcedure) exec.Executor {
	base := exec.NewBaseExecutor(b.ctx, v.Schema(), v.ID())
	base.SetInitCap(chunk.ZeroCapacity)
	sqlMode, ok := b.ctx.GetSessionVars().GetSystemVar(vardef.SQLModeVar)
	if !ok {
		b.err = errors.New("unknown system var " + vardef.SQLModeVar)
	}
	e := &ProcedureExec{
		BaseExecutor:        base,
		Statement:           nil,
...
func (e *ProcedureExec) callProcedure(ctx context.Context, s *ast.CallStmt) (funcRet *types.Datum, err error) {
	...
	if needChangeUser {
		...
		if e.definerUser != "" {
			...
			var success bool
			definer.AuthUsername, definer.AuthHostname, success = pm.MatchIdentity(ctx, definer.Username, definer.Hostname, false)
			if !success {
			...
			e.Ctx().GetSessionVars().ActiveRoles = pm.GetDefaultRoles(ctx, e.definerUser, e.definerHost)
		} else {
	...
	sysvar := variable.GetSysVar(vardef.LowerCaseTableNames)
	...
	if key != e.Ctx().GetSessionVars().CurrentDB {
		...
		e.Ctx().GetSessionVars().CurrentDBCI = ast.NewCIStr(key)
	}
	...
	sqlModeSave, ok := e.Ctx().GetSessionVars().GetSystemVar(vardef.SQLModeVar)
	if !ok {
	...
	err = e.Ctx().GetSessionVars().SetSystemVar(vardef.SQLModeVar, e.ProcedureSQLMod)
	defer func() {
		restoreErr := e.Ctx().GetSessionVars().SetSystemVar(vardef.SQLModeVar, sqlModeSave)
		if restoreErr != nil {
